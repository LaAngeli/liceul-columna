{{-- Coada cererilor tipice (secretariat): vederi pe starea cererii („De procesat" / „Arhivă") →
     carduri pe TIPUL cererii (învoire / adeverință / transfer / contestație / ședință) → tabelul
     în context. Cererile se depun din cabinet; panoul le procesează, cu comentariu vizibil familiei. --}}
<x-filament-panels::page>
    <div class="space-y-6">
        <x-filament::tabs :label="__('panel.document_nav.aria')">
            @foreach ($this->requestsViewPills() as $pill)
                <x-filament::tabs.item
                    :active="$pill['active']"
                    :badge="$pill['count']"
                    wire:click="setRequestsView('{{ $pill['key'] }}')"
                >
                    {{ $pill['label'] }}
                </x-filament::tabs.item>
            @endforeach
        ...
        @elseif ($this->queueIsEmpty())
            <div class="flex flex-col items-center gap-3 rounded-xl bg-white px-6 py-12 text-center shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                ...
                <p class="text-sm font-medium text-gray-950 dark:text-white">
                    {{ __('panel.document_nav.queue_empty_title') }}
                </p>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ __('panel.document_nav.queue_empty_description') }}
                </p>
            </div>
        @else
